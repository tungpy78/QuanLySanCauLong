...
export class AuthService {
    ...
    static async login(data: any, allowedRoles: string[]) {
        ...
        if (!isMatch) throw new ApiError('Email hoặc mật khẩu không đúng', 401);

        if (!user.is_active) throw new ApiError('Tài khoản của bạn đã bị khóa', 403);
    ...
    static async refreshAccessToken(clientRefreshToken: string) {
        if (!clientRefreshToken) throw new ApiError('Vui lòng đăng nhập', 401);
        ...
        if (!tokenRecord) throw new ApiError('Refresh Token không hợp lệ', 401);
        if (tokenRecord.revoked) throw new ApiError('Phiên đăng nhập đã bị thu hồi. Vui lòng đăng nhập lại!', 401);
        if (dayjs().isAfter(dayjs(tokenRecord.expires_at))) {
            throw new ApiError('Phiên đăng nhập đã hết hạn', 401);
        }

        const user = await models.User.findByPk(tokenRecord.user_id);
        if (!user || !user.is_active) throw new ApiError('Tài khoản không tồn tại hoặc bị khóa', 401);
