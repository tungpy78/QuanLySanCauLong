...
export class RevenueService {
     ...
     */
    static async resolveFilters(from, to, facilityId) {
    ...
    static async getSummary(filters) {
        const { fromStr, toStr, fromDate, toDate, scope, facilityId, facilityName } = await this.resolveFilters(filters.from, filters.to, filters.facility_id);
        const repoParams = {
            from: fromDate,
            to: toDate
        };
     ...
     */
    static async getChart(filters) {
        const { fromStr, toStr, fromDate, toDate, facilityId } = await this.resolveFilters(filters.from, filters.to, filters.facility_id);
        const repoParams = {
            from: fromDate,
            to: toDate
        };
     ...
     */
    static async getBreakdown(filters) {
        const { fromDate, toDate, facilityId } = await this.resolveFilters(filters.from, filters.to, filters.facility_id);
        const repoParams = {
            from: fromDate,
            to: toDate
        };
