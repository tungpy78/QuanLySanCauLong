...
export class PaymentService {
    static async validateVNPayOrder(query) {
        const isValidSignature = VNPayUtils.verifyIpnSignature(query);
        if (!isValidSignature) {
            return {
                RspCode: '97',
        ...
        const orderId = Number(query.vnp_TxnRef.replace('ORDER_', ''));
        ...
        if (payment.status === 'paid') {
            throw new ApiError('Order already confirmed', 400);
        }
        if (order.status !==
            'pending_payment') {
    ...
    static async completeVNPayPayment(data, query) {
        const t = await sequelize.transaction();
        try {
            const { order, payment } = data;
            if (query.vnp_ResponseCode ===
                '00') {
                await this.deductOrderInventory(order.id, order.facility_id, t);
                await payment.update({
                    status: 'paid',
                    provider_ref: query.vnp_TransactionNo,
                    paid_at: new Date()
                }, {
                    transaction: t
                });
            ...
            await t.rollback();
            console.error('POS VNPay IPN Error:', error);
            return {
                RspCode: '99',
    ...
    static async processVNPayIPN(vnpayQuery) {
        ...
        try {
            const booking = await models.Booking.findByPk(bookingId, { transaction: t });
    ...
    static getVNPayReturnHtml(vnpayQuery) {
        const vnp_ResponseCode = vnpayQuery.vnp_ResponseCode;
        if (vnp_ResponseCode === '00') {
            return `
                <html lang="vi">
    ...
    static async payCash(orderId) {
        ...
        try {
            ...
            if (payment && payment.status === 'paid') {
                throw new ApiError('Đơn hàng đã thanh toán', 400);
            }
            if (payment) {
                await payment.update({
                    status: 'paid',
            ...
            await orderRepository
                .updateStatus(order.id, 'pending_pickup');
            await t.commit();
            return {
    ...
    static async deductOrderInventory(orderId, facilityId, transaction) {
        const items = await orderItemRepository
            .findByOrderId(orderId, transaction);
    ...
    static async processPosOrderVNPayIPN(query) {
        const validationResult = await this.validateVNPayOrder(query);
        if ('RspCode' in validationResult) {
            return validationResult;
        }
        return this.completeVNPayPayment(validationResult, query);
    }
}
