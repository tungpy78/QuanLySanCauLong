...
export class OrderService {
    static async createOrder(userId, data) {
        ...
        try {
            ...
            const variantIds = items.map((item) => item.variant_id || item.product_variant_id);
            ...
            if (!targetFacilityId) {
                const firstFacility = await models.Facility.findOne({
                    attributes: ['id']
                });
                if (!firstFacility) {
            ...
            const order = await orderRepository.createOrder({
                ...
                note: `Khách: ${customer_name ||
                    'N/A'} - ${customer_phone ||
                    'N/A'}. Đ/c: ${shipping_address ||
                    'N/A'}. ${note || ''}`
            }, t);
            ...
            if (payment_method) {
                ...
                paymentResult =
                    await strategy.process(order, t);
            }
            await t.commit();
            return {
                order,
    ...
    static async cancelOrder(orderId, userId) {
        ...
        if (order.status !==
            'pending_payment') {
            throw new ApiError('Chỉ có thể hủy đơn hàng đang chờ xử lý', 400);
        }
        await order.update({
            status: 'cancelled'
        });
    ...
    static async getPendingPickupOrders() {
        return orderRepository
            .getByStatus('pending_pickup');
    }
    static async getPendingPaymentOrders() {
        return orderRepository
            .getByStatus('pending_payment');
    }
    static async completeOrder(orderId) {
        ...
        if (order.status !==
            'pending_pickup') {
            throw new ApiError('Đơn chưa sẵn sàng giao', 400);
        }
    ...
    static async confirmOrder(orderId) {
        ...
        if (order.status !==
            'pending_payment') {
            throw new ApiError('Trạng thái đơn không hợp lệ', 400);
        }
        await orderRepository.updateStatus(orderId, 'pending_pickup');
        return {
