...
export class BookingService {
    static async getAvailableCourts(startDateTime, endDateTime, courtType) {
        ...
        const results = await Promise.all(availableCourts.map(async (court) => {
            try {
                const totalPrice = await PricingService.calculateTotalPrice(court.facility_id, court.court_type, startDateTime, endDateTime);
                return {
                    ...court.toJSON(),
    ...
    static async getDailyBookedSlots(facilityId, date, courtType) {
        ...
        courts.forEach(court => {
            ...
            for (let h = START_HOUR; h < END_HOUR; h++) {
                ...
                const { totalPrice } = pricingContext.executeCalculation(priceConfigs, slotStart.toDate(), slotEnd.toDate());
                courtSlots.push({
    ...
    static async createBooking(userId, data) {
        ...
        const calculatedPrice = await PricingService.calculateTotalPrice(data.facility_id, court.court_type, startDateTime, endDateTime, userId);
    ...
    static async checkConflictingSlot(courtId, start, end, t) {
    ...
    static async checkSmartGapRules(courtId, start, end, t) {
        ...
        if (previousBooking) {
            ...
            if (gapBefore > 0 && gapBefore < MIN_DURATION_MINUTES) {
                throw new ApiError(`Không thể đặt! Sẽ tạo ra khoảng trống ${gapBefore} phút (từ ${dayjs(previousBooking.end_at).format('HH:mm')} đến ${dayjs(start).format('HH:mm')}) không đủ để người khác thuê.`, 400);
            }
        }
        ...
        if (nextBooking) {
            ...
            if (gapAfter > 0 && gapAfter < MIN_DURATION_MINUTES) {
                throw new ApiError(`Không thể đặt! Sẽ tạo ra khoảng trống ${gapAfter} phút (từ ${dayjs(end).format('HH:mm')} đến ${dayjs(nextBooking.start_at).format('HH:mm')}) không đủ để người khác thuê.`, 400);
            }
        }
    }
    ...
    static async getAllBookings(facilityId) {
        const whereCondition = {};
        if (facilityId) {
    ...
    static async updateBookingStatus(id, data) {
        const t = await sequelize.transaction();
        try {
            const booking = await bookingRepository.findById(id);
            if (!booking)
                throw new ApiError('Không tìm thấy lịch đặt này', 404);
    ...
    static async validateBookingTimes(date, startTime, endTime) {
    ...
    static async createBookingByHotline(data) {
        ...
        if (!user) {
            ...
            user = await (await import('./user.service.js')).UserService.createGuestUser(customer_phone, nameToSave, membership_type);
        }
    ...
    static async generateVNPayUrl(bookingId, ipAddr) {
        // 1. Tìm đơn hàng
