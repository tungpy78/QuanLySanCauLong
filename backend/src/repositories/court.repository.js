...
class CourtRepository extends BaseRepository {
    ...
    async findAllWithFacility(whereCondition = {}, orderCondition = [['created_at', 'DESC']]) {
    ...
    async findByIdWithFacility(id, whereCondition = {}) {
    ...
    async findDuplicateNameInFacility(name, facilityId, excludeId) {
        const whereCondition = { name, facility_id: facilityId };
        if (excludeId) {
